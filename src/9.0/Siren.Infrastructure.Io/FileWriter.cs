...
namespace Siren.Infrastructure.Io
{
    public class FileWriter : IFileWriter
    {
        ...
        public void Perform(string filePath, StringBuilder result, string markdownAnchor = null)
        {
            var exists = File.Exists(filePath);

            if (!exists)
            {
                _logger.LogInformation($"Existing file \"{filePath}\" not found, creating...");

                OverwriteFileContents(filePath, result.ToString());
                return;
            }

            if (string.IsNullOrEmpty(markdownAnchor))
            {
                _logger.LogInformation($"Markdown anchor \"{markdownAnchor}\" not specified, writing directly...");
            ...
            var markdownPosition =
                fileString.IndexOf(markdownAnchor, StringComparison.InvariantCultureIgnoreCase) + markdownAnchor.Length;

            if (markdownPosition < 0)
            {
                _logger.LogInformation($"Markdown anchor \"{markdownAnchor}\"not found in file; overwriting...");
            ...
            var sirenAnchorStartPosition = fileString.IndexOf(
            ...
            var sirenAnchorEndPosition = fileString.IndexOf(
            ...
            fileString = fileString.Remove(
                sirenAnchorStartPosition,
                sirenAnchorEndPosition - sirenAnchorStartPosition + lengthToTrim
            );

            fileString = fileString.Insert(sirenAnchorStartPosition, result.ToString());

            OverwriteFileContents(filePath, fileString);
        }
