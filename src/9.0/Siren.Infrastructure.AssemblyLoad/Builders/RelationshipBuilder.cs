...
namespace Siren.Infrastructure.AssemblyLoad.Builders
{
    public class RelationshipBuilder : IRelationshipBuilder
    {
        ...
        public bool IsApplicable(Instruction instr)
        {
            ...
            if (instr.Operand is not Instruction opInstr)
                return false;

            if (opInstr.OpCode != OpCodes.Callvirt)
                return false;

            if (opInstr.Operand is not MethodReference mr)
                return false;
        ...
        public ICollection<ExtractedRelationship> Process(Instruction instr, ICollection<ExtractedEntity> entities)
        {
            ...
            var extractedSource = currInstr.Operand.ToString().SplitNamespace();

            if (string.IsNullOrEmpty(extractedSource.Item2))
                return null;

            var matchedSourceEntity = GetMatchedEntity(entities, extractedSource.Item2, extractedSource.Item1);

            if (matchedSourceEntity == null)
                return null;
            ...
            if (currInstr.Operand is not MethodDefinition methodReference)
                return null;

            // Extract relationship indicators
            var relationshipInstrs = methodReference.Body.Instructions.ToList();
            ...
            if (relationshipInstrs.Count > 0)
            {
                ...
                for (var i = 0; i < relationshipInstrs.Count; i++)
                {
                    ...
                    if (mexInstr.Operand is not MethodReference mr)
                        continue;
                    ...
                    switch (mr.Name)
                    {
                        case "HasOne":
                            ...
                            newResult = new ExtractedRelationship { Source = matchedSourceEntity };
                            var entityInstr = mexInstr.StepPrevious(2);

                            if (entityInstr.OpCode != OpCodes.Ldstr)
                                break;

                            var (extractedNamespace, extractedEntityName) = entityInstr
                                .Operand.ToString()
                                .SplitNamespace();

                            if (!string.IsNullOrEmpty(extractedEntityName))
                            {
                                var matchedEntity = GetMatchedEntity(entities, extractedEntityName, extractedNamespace);
                        ...
                        case "IsRequired":
                            ...
                            break;
                        case "HasForeignKey":
                            var foreignKeyInstr = mexInstr.StepPrevious(2);

                            if (foreignKeyInstr.OpCode != OpCodes.Ldstr)
                                break;

                            var foreignKeyValue = foreignKeyInstr.Operand.ToString();

                            var matchedProperty = matchedSourceEntity.Properties.FirstOrDefault(o =>
                                o.PropertyName == foreignKeyValue
                            );
        ...
        ) => entities.FirstOrDefault(o => o.EntityName == extractedEntityName && o.Namespace == extractedNamespace);
    }
}
