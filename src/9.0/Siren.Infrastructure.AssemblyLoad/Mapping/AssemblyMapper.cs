...
namespace Siren.Infrastructure.AssemblyLoad.Mapping
{
    public class AssemblyMapper : IAssemblyMapper
    {
        ...
        )
        {
            ...
            var relationships = extractedRelationships
                .Select(o => new Relationship
                {
            ...
            _logger.LogInformation($"{entities.Count} entities mapped from {extractedEntities.Count} extracted");

            _logger.LogInformation(
                $"{relationships.Count} relationships mapped from {extractedRelationships.Count} extracted"
            );
