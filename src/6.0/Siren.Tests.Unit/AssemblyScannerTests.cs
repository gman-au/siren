	...
	public class AssemblyScannerTests
	{
		...
		private class TestContext
		{
			...
			public void AssertResult()
			{
				...
				Assert.Equal("Customer", entities[0].Name);
				Assert.Equal(4, entities[0].Properties.Count());
				
				Assert.Equal("Order", entities[1].Name);
				Assert.Equal(4, entities[1].Properties.Count());
			}
		}
	}
}